  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
      ...
      <View style={styles.header}>
        ...
        {user.readyStatus && <StatusBadge status={user.readyStatus} small />}
      </View>

      {/* Pace */}
      <View style={styles.paceRow}>
        <Ionicons name="timer-outline" size={14} color={theme.brand} />
        <Text style={[styles.pace, { color: theme.text }]}>{user.paceRange}</Text>
      </View>
      ...
      <View style={styles.tags}>
        ...
        {user.distances.slice(0, 2).map((d) => (
        ...
        {user.training && (
          <View style={[styles.tag, { backgroundColor: theme.warningLight }]}>
            <Text style={[styles.tagText, { color: theme.warning }]}>{user.training}</Text>
          </View>
        )}
      </View>
